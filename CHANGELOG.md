...
**Сборка разработки:** `173` — `build/173-development`, кодовая доработка проверена; ручная приёмка устройств открыта.
...
- Реальные iPhone PWA/Android, полный media/voice UX и числовой baseline устройств ещё требуют приёмки. Build 174 не начата; ускорение без замеров не заявляется.
