...
**Сборка разработки:** `174` — `build/174-development`, реализация и автоматическая регрессия завершены; приёмка физических устройств открыта.
...
- Далёкий anchor, reply и pin используют существующие `before/after` API. История прокручивается в обе стороны и удерживает рабочее окно до 300 сообщений; временная страница и неподтверждённые отправки описаны в отчёте.
...
- Проверки: `npm run check:169-174`, **17/17** сценариев регрессии 173 и **17/17** сценариев 174. Замеры выполнены на изолированном Linux/Chromium; ускорение на физических телефонах не заявляется.
- [Реализация, сырые замеры, ограничения покрытия и откат](docs/FPChat_Build174_Complete.md). `main` остаётся на 168; этап 175 не начат.
