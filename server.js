...
function serializeMessages(messages) {
  return hydrateMessages(messages).map((m) => ({
    ...m,
    created_at: toIsoUtc(m.created_at),
    delivered_at: toIsoUtc(m.delivered_at),
...
app.post('/api/invites/:inviteCode/join', (req, res) => {
  ...
  if (invite.revoked || invite.used_at || !invite.room_secret) return res.status(410).json({ error: 'invite expired or used' });
...
app.get('/api/rooms/:publicId/messages', (req, res) => {
  const room = q.findRoomByPublicId.get(req.params.publicId);
  if (!room) return res.status(404).json({ ok: false, error: 'room not found' });
  ...
  for (const participant of q.listParticipantsByRoom.all(room.id)) {
    ...
    for (const client of sockets) {
      ...
      try {
        ...
        if (participant.device_id !== sourceDeviceId) deliveredToRecipient = true;
      } catch {}
    }
  }
...
function markMessageReceived(room, recipientParticipantId, messageId) {
  ...
  const info = q.markDelivered.run(messageId);
...
async function handleTextMessage(ws, payload) {
  ...
  let row = q.findMessageByClientId.get(room.id, sender.id, clientMessageId);
  if (!row) {
    try {
      const result = q.createMessage.run(room.id, sender.id, ciphertext, iv, 'text', clientMessageId, replyToMessageId);
      row = q.findMessageById.get(Number(result.lastInsertRowid), room.id);
    } catch {
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (raw) => {
    ...
    if (payload.type === 'message:received' || (payload.type === 'message:received:bulk' && Array.isArray(payload.messageIds))) {
      const room = q.findRoomByPublicId.get(String(payload.roomId || ''));
      if (!room) return;
      const recipient = q.findParticipant.get(room.id, ws.deviceId);
      if (!recipient) return;
      const rawIds = payload.type === 'message:received' ? [payload.messageId] : payload.messageIds;
      const ids = [...new Set(rawIds.map((id) => Number(id)).filter((id) => Number.isSafeInteger(id) && id > 0))];
