/* Build 114: isolated pinned-messages screen enhancement. */
(() => {
  ...
  async function loadThumb(pin, card) {
    ...
    if (!first || !img || typeof fetchMediaThumbUrl !== 'function') return;
    const key = String(first.public_id || first.id || '');
    try {
      let url = key ? thumbCache.get(key) : '';
      if (!url) {
        url = await fetchMediaThumbUrl(first);
        if (url && key) thumbCache.set(key, url);
      }
  ...
  function cardFor(pin) {
    ...
    const thumb = pin.message?.type === 'media' && first
      ? `<span class="fp-pins114-thumb"><img class="fp-pins114-thumb-img" alt="" hidden><span class="fp-pins114-thumb-fallback">${first.media_kind === 'video' ? '▶' : '▧'}</span>${first.media_kind === 'video' ? '<span class="fp-pins114-play">▶</span>' : ''}${extra ? `<span class="fp-pins114-extra">+${extra}</span>` : ''}</span>`
      : '';
