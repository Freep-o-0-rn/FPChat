/* Build 184.6: presentation bridge with Build 169-180 owners and Layer/Gesture/Scroll/Viewport ownership. */
(() => {
  if (window.__fpBuild165UiInstalled) return;
  window.__fpBuild165UiInstalled = true;

  const BUILD_LABEL = 'Build 184.6';
  const currentScript = document.currentScript;
  const storageSuffix = (() => {
    try { return new URL(currentScript?.src || '', window.location.href).search || '?v=184.6'; }
    catch { return '?v=184.6'; }
  })();
  let voiceBlockNoticeUntil = 0;
  let voiceNoticeTimer = 0;

  // Build 169 diagnostics remain passive in Build 184.6.
  ...
  // Build 170 ownership primitives remain active under the Build 171 network owner in Build 184.6.
