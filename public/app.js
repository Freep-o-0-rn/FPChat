...
function connectWs(roomId,deviceId){setLocalConnectionState('connecting');if(state.ws)state.ws.close();const p=location.protocol==='https:'?'wss':'ws';state.ws=new WebSocket(`${p}://${location.host}?room=${roomId}&device=${encodeURIComponent(deviceId)}`);wsConnectStartedAt=Date.now();state.ws.onopen=()=>setLocalConnectionState('connected');state.ws.onerror=()=>setLocalConnectionState('disconnected');state.ws.onclose=()=>setLocalConnectionState('disconnected');state.ws.onmessage=async(ev)=>{const payload=JSON.parse(ev.data);if(payload.type==='message:new'){const chat=state.chats.find(c=>c.roomId===roomId)||{};const txt=await decryptText(payload.message.iv,payload.message.ciphertext).catch(()=>"[cannot decrypt]");const mine=payload.message.sender_device_id===deviceId;const inActive=state.roomId===roomId;const nearBottom=inActive?isMessagesAtBottom():false;const normalizedLast={lastMessage:txt,lastSender:payload.message.sender_name,lastActivity:new Date().toISOString()};if(!mine){if(!inActive){upsertChat(roomId,{...normalizedLast,unread:(chat.unread||0)+1});}else if(!nearBottom){const incomingId=Number(payload.message.id);if(Number.isInteger(incomingId)&&incomingId>0){pendingIncomingReadIds=[...new Set([...pendingIncomingReadIds,incomingId])];}const unreadCount=pendingIncomingReadIds.length;renderNewMessagesPill(unreadCount);upsertChat(roomId,{...normalizedLast,unread:unreadCount});}else{upsertChat(roomId,{...normalizedLast,unread:0});}}else{upsertChat(roomId,{...normalizedLast,unread:chat.unread||0});}
if(inActive){const box=document.getElementById('messages');appendMessage(box,payload.message,txt,mine,nearBottom||mine);if(!mine&&nearBottom){markMessageRead(payload.message.id);}if(!mine&&!nearBottom){renderNewMessagesPill(pendingIncomingReadIds.length);}recomputePendingUnread();updateUnreadIndicators();}else{updatePushBadge();}
