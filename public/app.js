...
let activeChatDeviceId=null;let pendingIncomingReadIds=[];const pendingReadQueue=new Map();let unreadVisibleObserver=null;let appWsSeq=0;let activeAppWsSeq=0;let appWsConnectInFlight=null;
...
function queueReadIds(roomId,messageIds){if(!roomId||!Array.isArray(messageIds)||!messageIds.length)return;const ids=messageIds.map(Number).filter((id)=>Number.isInteger(id)&&id>0);if(!ids.length)return;if(!pendingReadQueue.has(roomId)){pendingReadQueue.set(roomId,new Set());}const set=pendingReadQueue.get(roomId);ids.forEach((id)=>set.add(id));}
function flushPendingReads(roomId=state.roomId){if(!roomId||!activeChatDeviceId)return false;if(!state.ws||state.ws.readyState!==WebSocket.OPEN)return false;const set=pendingReadQueue.get(roomId);if(!set||!set.size)return true;const messageIds=[...set].map(Number).filter((id)=>Number.isInteger(id)&&id>0);if(!messageIds.length){pendingReadQueue.delete(roomId);return true;}state.ws.send(JSON.stringify({type:'message:read:bulk',roomId,messageIds}));pendingReadQueue.delete(roomId);return true;}
function markIncomingMessagesRead(roomId,deviceId,messageIds){if(!roomId||!deviceId||!Array.isArray(messageIds)||!messageIds.length)return false;const ids=[...new Set(messageIds.map(Number).filter((id)=>Number.isInteger(id)&&id>0))];if(!ids.length)return false;queueReadIds(roomId,ids);const flushed=flushPendingReads(roomId);if(!flushed){ensureWsConnected(deviceId).then(()=>{flushPendingReads(roomId);}).catch(()=>{});}return flushed;}
...
updateReplyComposerBar();
box.addEventListener('scroll',()=>{if(isMessagesAtBottom()){const unreadEls=[...box.querySelectorAll('.msg[data-incoming="1"][data-read="0"]')];if(unreadEls.length){unreadEls.forEach((el)=>{markMessageRead(el.dataset.messageId||el.dataset.id);});}}recomputePendingUnread();updateUnreadIndicators();
updateReplyComposerBar();});
...
const restoreWsOnResume=()=>{if(state.roomId&&activeChatDeviceId){ensureWsConnected(activeChatDeviceId).then(()=>{flushPendingReads(state.roomId);}).catch(()=>{});}};
const handleAppResume=()=>{restoreWsOnResume();checkAppVersionOnEntry();};
window.addEventListener('focus',handleAppResume);
document.addEventListener('visibilitychange',()=>{sendClientState();if(document.visibilityState==='visible'){handleAppResume();flushPendingReads(state.roomId);}});window.addEventListener('focus',sendClientState);window.addEventListener('pageshow',()=>{sendClientState();flushPendingReads(state.roomId);});
