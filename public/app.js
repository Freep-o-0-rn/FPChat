...
function renderNewMessagesPill(count){const pill=document.getElementById('newMessagesPill');if(!pill)return;if(count>0){pill.textContent=`${count} ${count===1?'новое сообщение':'новых сообщения'} ↓`;pill.classList.remove('hidden');}else{pill.classList.add('hidden');}}
function renderChatView(messages,deviceId){messages=Array.isArray(messages)?messages:[];activeChatDeviceId=deviceId;pendingIncomingReadIds=[];pendingPillCount=0;els.content.innerHTML=`<div class='chat-header'><div><strong>${state.roomNames[state.roomId]||`Комната ${shortId(state.roomId)}`}</strong><div id='presenceLine' class='presence-line'></div><div id='connectionWarning' class='connection-warning hidden'></div></div><div class='chat-header-actions'><button id='backMob' class='mobile-only btn btn-icon' aria-label='Назад'>←</button><button id='reloadBtn' class='btn btn-icon' aria-label='Обновить'>↻</button><button id='menuBtn' class='btn btn-icon' aria-label='Меню чата'>⋮</button></div></div><div class='messages' id='messages'></div><button id='newMessagesPill' class='new-messages-pill hidden' type='button'></button><form class='send' id='sendForm'><textarea id='msgInput' placeholder='Сообщение'></textarea><button id='sendBtn' class='btn-send' type='submit' disabled>➤</button></form>`; document.getElementById('backMob')?.addEventListener('click',()=>setView('chats')); document.getElementById('reloadBtn').onclick=()=>window.location.reload(); document.getElementById('menuBtn').onclick=(e)=>{e.preventDefault();e.stopPropagation();const rect=e.currentTarget.getBoundingClientRect();showRoomMenu(state.roomId,rect.right,rect.bottom+6)};
const box=document.getElementById('messages'); let prev=''; const initialUnread=[]; messages.forEach(async m=>{const d=new Date(m.created_at).toDateString(); if(d!==prev){prev=d; const sep=document.createElement('div');sep.className='date-sep';sep.textContent=d===new Date().toDateString()?'Сегодня':d;box.appendChild(sep);} const mine=m.sender_device_id===deviceId; if(!mine&&m.status!=='read')initialUnread.push(m.id); const txt=await decryptText(m.iv,m.ciphertext).catch(()=>"[cannot decrypt]"); appendMessage(box,m,txt,mine,false);}); box.scrollTop=box.scrollHeight; renderNewMessagesPill(0); if(initialUnread.length)markIncomingMessagesRead(state.roomId,deviceId,initialUnread);
box.addEventListener('scroll',()=>{if(isMessagesAtBottom()&&pendingIncomingReadIds.length){markIncomingMessagesRead(state.roomId,deviceId,pendingIncomingReadIds);pendingIncomingReadIds=[];pendingPillCount=0;renderNewMessagesPill(0);}});
document.getElementById('newMessagesPill').onclick=()=>{box.scrollTop=box.scrollHeight;if(pendingIncomingReadIds.length){markIncomingMessagesRead(state.roomId,deviceId,pendingIncomingReadIds);pendingIncomingReadIds=[];}pendingPillCount=0;renderNewMessagesPill(0);};
...
function appendMessage(box,m,txt,mine,autoScroll=true){const w=document.createElement('div');w.className=`bubble-wrap ${mine?'mine':''}`;w.innerHTML=`<div class='bubble'><div><b>${m.sender_name}</b></div><div>${txt}</div><div class='meta'>${fmtTime(m.created_at)} ${mine?deliveryIcon(m.status):''}</div></div>`;w.dataset.id=m.id;w.dataset.createdAt=m.created_at;box.appendChild(w);if(autoScroll)box.scrollTop=box.scrollHeight;}
function connectWs(roomId,deviceId){setLocalConnectionState('connecting');if(state.ws)state.ws.close();const p=location.protocol==='https:'?'wss':'ws';state.ws=new WebSocket(`${p}://${location.host}?room=${roomId}&device=${encodeURIComponent(deviceId)}`);wsConnectStartedAt=Date.now();state.ws.onopen=()=>setLocalConnectionState('connected');state.ws.onerror=()=>setLocalConnectionState('disconnected');state.ws.onclose=()=>setLocalConnectionState('disconnected');state.ws.onmessage=async(ev)=>{const payload=JSON.parse(ev.data);const chat=state.chats.find(c=>c.roomId===roomId)||{}; if(payload.type==='message:new'){const txt=await decryptText(payload.message.iv,payload.message.ciphertext).catch(()=>"[cannot decrypt]"); const mine=payload.message.sender_device_id===deviceId; const inActive=state.roomId===roomId; upsertChat(roomId,{lastMessage:txt,lastSender:payload.message.sender_name,lastActivity:new Date().toISOString(),unread: inActive?chat.unread||0:(chat.unread||0)+(mine?0:1)}); if(inActive){const box=document.getElementById('messages');const nearBottom=isMessagesAtBottom();appendMessage(box,payload.message,txt,mine,nearBottom||mine); if(!mine){if(nearBottom){markIncomingMessagesRead(roomId,deviceId,[payload.message.id]);}else{pendingIncomingReadIds.push(payload.message.id);pendingPillCount+=1;renderNewMessagesPill(pendingPillCount);upsertChat(roomId,{unread:(chat.unread||0)+1});}}}
if(!mine)notifyIncoming(payload.message.sender_name,txt,roomId);} updatePushBadge(); if(payload.type==='presence:update'&&payload.deviceId){state.presence[payload.deviceId]={deviceId:payload.deviceId,displayName:payload.displayName,online:Boolean(payload.online),lastSeenAt:payload.lastSeenAt||null};renderPresenceStatus();} if(payload.type==='message:status'){document.querySelectorAll('.bubble-wrap').forEach(el=>{if(Number(el.dataset.id)===payload.messageId){const meta=el.querySelector('.meta');const createdAt=el.dataset.createdAt;const base=fmtTime(createdAt);const hasIcon=meta.textContent.includes('✓')||meta.textContent.includes('⏳');meta.innerHTML=hasIcon?`${base} ${deliveryIcon(payload.status)}`:base;}});}};}
...
function renderJoin(){els.content.innerHTML=`<div class='panel'><h2>Присоединиться к чату</h2><label>Invite-ссылка</label><textarea id='joinInviteInput' placeholder='Вставьте invite-ссылку'></textarea><div class='panel-actions'><button id='joinBtn' class='btn btn-primary'>Присоединиться</button><button id='backBtn' class='btn btn-secondary'>Назад</button></div></div>`; document.getElementById('backBtn').onclick=()=>setView('chats'); document.getElementById('joinBtn').onclick=async()=>{const parsed=parseInviteInput(document.getElementById('joinInviteInput').value); if(parsed?.error==='empty'){alert('Вставьте invite-ссылку');return;} if(parsed?.error==='missing_secret'){alert('В invite-ссылке нет ключа доступа');return;} if(!parsed||parsed?.error){alert('Некорректная invite-ссылка');return;} state.nick=localStorage.getItem(STORAGE.nick)||state.nick; const {roomId,secret}=parsed; const stored=STORAGE.get(STORAGE.roomState(roomId)); STORAGE.set(STORAGE.roomState(roomId),{...stored,secret,deviceId:stored?.deviceId||crypto.randomUUID()}); upsertChat(roomId,{}); await openChat(roomId);};}
