...
function appendMessage(box,m,txt,mine,autoScroll=true){const w=document.createElement('div');w.className=`bubble-wrap msg ${mine?'mine':''}`;const isIncoming=!mine;const isRead=mine?1:(m.status==='read'?1:0);const replyMeta=m.reply_to_message_id?getMessageReplyMeta(m.reply_to_message_id):null;const replyHtml=replyMeta?`<button type="button" class="reply-block" data-reply-message-id="${replyMeta.messageId}"><div class="reply-block-author">${replyMeta.author}</div><div class="reply-block-preview">${replyMeta.preview}</div></button>`:'';w.innerHTML=`<div class='bubble'><div><b>${m.sender_name}</b></div>${replyHtml}<div>${txt}</div><div class='meta'>${formatMessageTime(m.created_at)} ${mine?deliveryIcon(m.status):''}</div></div>`;const swipeIcon=document.createElement('div');swipeIcon.className='swipe-reply-icon';swipeIcon.textContent='↩';w.appendChild(swipeIcon);w.dataset.id=m.id;w.dataset.createdAt=m.created_at;w.dataset.messageId=String(m.id);w.dataset.incoming=isIncoming?'1':'0';w.dataset.read=String(isRead);w.querySelector('.reply-block')?.addEventListener('click',(e)=>{e.preventDefault();e.stopPropagation();const id=Number(e.currentTarget.dataset.replyMessageId);void findAndFocusReplyMessage(id);});w.addEventListener('contextmenu',(e)=>{e.preventDefault();e.stopPropagation();showMessageReplyMenu(m.id,e.clientX,e.clientY);});let swipeState=null;w.addEventListener('touchstart',(e)=>{if(!isMobileViewport())return;const touch=e.touches?.[0];if(!touch)return;swipeState={startX:touch.clientX,startY:touch.clientY,dx:0,canceled:false,tracking:true};w.classList.remove('swipe-reset');},{passive:true});w.addEventListener('touchmove',(e)=>{if(!swipeState?.tracking||swipeState.canceled)return;const touch=e.touches?.[0];if(!touch)return;const dx=touch.clientX-swipeState.startX;const dy=touch.clientY-swipeState.startY;swipeState.dx=dx;if(Math.abs(dy)>SWIPE_CANCEL_VERTICAL){swipeState.canceled=true;w.style.transform='translateX(0px)';swipeIcon.style.opacity='0';swipeIcon.style.transform='translateY(-50%) scale(.8)';return;}if(dx<0){const swipeDistance=Math.abs(dx);const progress=Math.min(swipeDistance/SWIPE_REPLY_THRESHOLD,1);w.style.transform=`translateX(-${Math.min(swipeDistance,72)}px)`;swipeIcon.style.opacity=String(progress);swipeIcon.style.transform=`translateY(-50%) scale(${0.8+progress*0.2})`;w.classList.add('swiping');}}, {passive:true});w.addEventListener('touchend',()=>{if(!swipeState){return;}const shouldReply=!swipeState.canceled&&swipeState.dx<=-SWIPE_REPLY_THRESHOLD;w.classList.remove('swiping');w.classList.add('swipe-reset');w.style.transform='translateX(0px)';swipeIcon.style.opacity='0';swipeIcon.style.transform='translateY(-50%) scale(.8)';if(shouldReply){setSelectedReply(state.roomId,getMessageReplyMeta(m.id));navigator.vibrate?.(10);}swipeState=null;});w.addEventListener('touchcancel',()=>{swipeState=null;w.classList.remove('swiping');w.classList.add('swipe-reset');w.style.transform='translateX(0px)';swipeIcon.style.opacity='0';swipeIcon.style.transform='translateY(-50%) scale(.8)';});box.appendChild(w);const previewText=txt==='[cannot decrypt]'?'Сообщение недоступно':makeReplyPreview(txt);messageCache.set(Number(m.id),{id:Number(m.id),author:m.sender_name,text:txt,preview:previewText,kind:'text'});if(isIncoming&&!isRead){if(!autoScroll){pendingIncomingReadIds.push(Number(m.id));}observeUnreadMessage(w);}if(autoScroll)box.scrollTop=box.scrollHeight;}
