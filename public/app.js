...
function appendMessage(box,m,txt,mine,autoScroll=true){const w=document.createElement('div');w.className=`bubble-wrap msg ${mine?'mine':''}`;const isIncoming=!mine;const isRead=mine?1:(m.status==='read'?1:0);w.dataset.id=m.id;w.dataset.createdAt=m.created_at;w.dataset.messageId=String(m.id);w.dataset.incoming=isIncoming?'1':'0';w.dataset.read=String(isRead);const replyMeta=m.reply_to_message_id?getMessageReplyMeta(m.reply_to_message_id):null;const replyHtml=replyMeta?`<button type="button" class="reply-block" data-reply-message-id="${replyMeta.messageId}"><div class="reply-block-author">${replyMeta.author}</div><div class="reply-block-preview">${replyMeta.preview}</div></button>`:'';const isMedia=m.type==='media';const mediaList=Array.isArray(m.media)?m.media:[];const caption=(txt||'').trim();const captionHtml=caption?`<div>${caption}</div>`:'';const mediaGridClass=mediaList.length===1?'one':(mediaList.length<=4?'few':'many');const contentHtml=isMedia?`<div class="media-grid ${mediaGridClass}">${mediaList.map((item,idx)=>`<button type="button" class="media-tile" data-media-index="${idx}"><img class="media-thumb" alt="media"><span class="media-video-badge ${item.media_kind==='video'?'':'hidden'}">▶</span></button>`).join('')}</div>${captionHtml}`:`<div>${txt}</div>`;w.innerHTML=`<div class='bubble'><div><b>${m.sender_name}</b></div>${replyHtml}${contentHtml}<div class='meta'>${formatMessageTime(m.created_at)} ${mine?deliveryIcon(m.status):''}</div></div>`;w.querySelector('.reply-block')?.addEventListener('click',(e)=>{e.preventDefault();e.stopPropagation();const id=Number(e.currentTarget.dataset.replyMessageId);void findAndFocusReplyMessage(id);});w.addEventListener('contextmenu',(e)=>{e.preventDefault();e.stopPropagation();showMessageReplyMenu(e,m);});const swipeIcon=document.createElement('div');swipeIcon.className='swipe-reply-icon';swipeIcon.textContent='↩';w.appendChild(swipeIcon);let touchStartX=0,touchStartY=0,currentDx=0,tracking=false;w.addEventListener('touchstart',(e)=>{if(e.touches.length!==1)return;const t=e.touches[0];touchStartX=t.clientX;touchStartY=t.clientY;currentDx=0;tracking=true;w.classList.add('swiping');},{passive:true});w.addEventListener('touchmove',(e)=>{if(!tracking||e.touches.length!==1)return;const t=e.touches[0];const dx=t.clientX-touchStartX;const dy=t.clientY-touchStartY;if(Math.abs(dy)>Math.abs(dx)){tracking=false;w.style.transform='';return;}currentDx=Math.max(-110,Math.min(0,dx));w.style.transform=`translateX(${currentDx}px)`;if(currentDx<-40)w.classList.add('reply-swipe-ready');else w.classList.remove('reply-swipe-ready');},{passive:true});const finishSwipe=()=>{if(!tracking&&currentDx===0)return;const shouldReply=currentDx<-72;w.style.transform='';w.classList.remove('swiping','reply-swipe-ready');tracking=false;currentDx=0;if(shouldReply)prepareReplyToMessage(m);};w.addEventListener('touchend',finishSwipe);w.addEventListener('touchcancel',finishSwipe);if(isMedia){w.querySelectorAll('.media-tile').forEach(async(el)=>{const idx=Number(el.dataset.mediaIndex);const item=mediaList[idx];const img=el.querySelector('img');const u=await fetchMediaThumbUrl(item);if(u)img.src=u;el.addEventListener('click',(e)=>{e.preventDefault();e.stopPropagation();openMediaViewer(mediaList,idx);});});}
