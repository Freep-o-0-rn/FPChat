...
let detachMobileComposerViewportSync=null;
function isTouchMobile(){return isMobileViewport()&&('ontouchstart'in window||navigator.maxTouchPoints>0);}
function setupMobileComposerViewportSync(composerEl,inputEl){
  detachMobileComposerViewportSync?.();
  detachMobileComposerViewportSync=null;
  if(!composerEl||!inputEl||!isTouchMobile()||!window.visualViewport)return;
  const vv=window.visualViewport;
  let rafId=0;
  const applyOffset=()=>{
    rafId=0;
    const keyboardLikeGap=Math.max(0,window.innerHeight-(vv.height+vv.offsetTop));
    const offset=keyboardLikeGap>24?Math.round(keyboardLikeGap):0;
    composerEl.style.setProperty('--composer-kb-offset',`${offset}px`);
  };
  const schedule=()=>{if(rafId)cancelAnimationFrame(rafId);rafId=requestAnimationFrame(applyOffset);};
  vv.addEventListener('resize',schedule);
  vv.addEventListener('scroll',schedule);
  inputEl.addEventListener('focus',schedule);
  inputEl.addEventListener('blur',schedule);
  schedule();
  detachMobileComposerViewportSync=()=>{
    vv.removeEventListener('resize',schedule);
    vv.removeEventListener('scroll',schedule);
    inputEl.removeEventListener('focus',schedule);
    inputEl.removeEventListener('blur',schedule);
    if(rafId)cancelAnimationFrame(rafId);
    composerEl.style.removeProperty('--composer-kb-offset');
  };
}
function setChatMode(enabled){
  if(!enabled&&detachMobileComposerViewportSync){
    detachMobileComposerViewportSync();
    detachMobileComposerViewportSync=null;
  }
  els.content?.classList.toggle('chat-mode',Boolean(enabled));
}
function setView(v){state.view=v; setActiveNav(v); closeMobileMenu(); setChatMode(false); if(v==='chats'){showListPane(); renderMainChatsPlaceholder(); return;} if(v==='create'){renderCreate(); showContentPane(); return;} if(v==='restore'){renderRestore(); showContentPane(); return;} if(v==='join'){renderJoin(); showContentPane(); return;} if(v==='settings'){renderSettings(); showContentPane();}}
...
async function joinByInviteText(text){
  ...
  const recoveryCode=stored?.recoveryCode||generateRecoveryCode();
  const recovery=await buildRecoveryPayload(recoveryCode,secret);
  let res;
  try{
    res=await fetch(`/api/rooms/${roomId}/join`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({displayName:state.nick,deviceId,recoverySalt:recovery.recoverySalt,recoveryVerifier:recovery.recoveryVerifier,recoverySecretIv:recovery.recoverySecretIv,recoverySecretCiphertext:recovery.recoverySecretCiphertext})});
  }catch{
  ...
  STORAGE.set(STORAGE.roomState(roomId),{...stored,secret,deviceId,recoveryCode});
...
async function renderChatView(messages,deviceId){messages=Array.isArray(messages)?messages:[];activeChatDeviceId=deviceId;pendingIncomingReadIds=[];if(unreadVisibleObserver){unreadVisibleObserver.disconnect();unreadVisibleObserver=null;}setChatMode(true);els.content.innerHTML=`<div class='chat-header'><div><strong>${state.roomNames[state.roomId]||`Комната ${shortId(state.roomId)}`}</strong><div id='presenceLine' class='presence-line'></div><div id='connectionWarning' class='connection-warning hidden'></div></div><div class='chat-header-actions'><button id='backMob' class='mobile-only btn btn-icon' aria-label='Назад'>←</button><button id='reloadBtn' class='btn btn-icon' aria-label='Обновить'>↻</button><button id='menuBtn' class='btn btn-icon' aria-label='Меню чата'>⋮</button></div></div><div class='messages' id='messages'></div><button id='newMessagesPill' class='new-messages-pill hidden' type='button'></button><form class='send' id='sendForm'><button id='plusBtn' class='btn-plus' type='button' aria-label='Добавить медиа'><svg viewBox='0 0 24 24' aria-hidden='true'><path d='M12 5v14M5 12h14' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round'/></svg></button><div class='composer-input-wrap'><textarea id='msgInput' placeholder='Сообщение'></textarea><button id='emojiBtn' class='btn-emoji' type='button' aria-label='Эмодзи'><svg viewBox='0 0 24 24' aria-hidden='true'><circle cx='12' cy='12' r='9' fill='none' stroke='currentColor' stroke-width='1.8'/><circle cx='9' cy='10' r='1.2' fill='currentColor'/><circle cx='15' cy='10' r='1.2' fill='currentColor'/><path d='M8.5 14c.9 1.2 2.1 1.8 3.5 1.8s2.6-.6 3.5-1.8' fill='none' stroke='currentColor' stroke-width='1.8' stroke-linecap='round'/></svg></button></div><button id='sendBtn' class='btn-send' type='submit' disabled aria-label='Отправить сообщение'><svg viewBox='0 0 24 24' aria-hidden='true'><path d='M4 11.8L20 4l-5.2 16-3.1-6.2L4 11.8z' fill='currentColor'/></svg></button></form>`; document.getElementById('backMob')?.addEventListener('click',()=>setView('chats')); document.getElementById('reloadBtn').onclick=()=>window.location.reload(); document.getElementById('menuBtn').onclick=(e)=>{e.preventDefault();e.stopPropagation();const rect=e.currentTarget.getBoundingClientRect();showRoomMenu(state.roomId,rect.right,rect.bottom+6)};
...
const form=document.getElementById('sendForm'),input=document.getElementById('msgInput'),sendBtn=document.getElementById('sendBtn'),plusBtn=document.getElementById('plusBtn'),emojiBtn=document.getElementById('emojiBtn'); if(form&&input&&sendBtn){setupMobileComposerViewportSync(form,input);const toastSoon=()=>showInAppToast({roomId:state.roomId,roomName:'FPChat',sender:'',text:'Скоро'});const resizeInput=()=>{input.style.height='auto';const lineHeight=parseFloat(getComputedStyle(input).lineHeight)||20;const maxHeight=Math.round(lineHeight*8+22);const minHeight=46;const next=Math.max(minHeight,Math.min(input.scrollHeight,maxHeight));input.style.height=next+'px';input.style.overflowY=input.scrollHeight>maxHeight?'auto':'hidden';};const resetInput=()=>{input.value='';input.style.height='';input.style.overflowY='hidden';};const syncSendBtn=()=>{sendBtn.disabled=!input.value.trim();}; input.addEventListener('input',()=>{resizeInput();syncSendBtn();}); input.addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();form.requestSubmit();}}); plusBtn?.addEventListener('click',toastSoon); emojiBtn?.addEventListener('click',toastSoon); form.onsubmit=async(e)=>{e.preventDefault();const t=input.value.trim();if(!t)return;const ok=await ensureWsConnected(activeChatDeviceId);if(!ok||!state.ws||state.ws.readyState!==WebSocket.OPEN){alert('Нет соединения. Попробуйте обновить чат.');return;}const enc=await encryptText(t); state.ws.send(JSON.stringify({type:'message:new',roomId:state.roomId,...enc,notificationPreview:t.slice(0,80)})); resetInput(); syncSendBtn();}; syncSendBtn();resizeInput();}}
...
const creatorDeviceId=crypto.randomUUID();const res=await fetch('/api/rooms',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({creatorDeviceId,recoverySalt:recSalt,recoveryVerifier:b64.encode(dig),recoverySecretIv:b64.encode(iv),recoverySecretCiphertext:b64.encode(c)})}); const data=await res.json(); STORAGE.set(STORAGE.roomState(data.publicId),{secret,deviceId:creatorDeviceId,recoveryCode:rec}); upsertChat(data.publicId,{}); if(state.notif.enabled)syncRoomPushSubscription(data.publicId).catch(()=>{}); if(state.notif.enabled)syncRoomPushSubscription(data.publicId).catch(()=>{});
