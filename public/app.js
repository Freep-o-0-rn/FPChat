...
const form=document.getElementById('sendForm'),input=document.getElementById('msgInput'),sendBtn=document.getElementById('sendBtn'); if(form&&input&&sendBtn){const syncSendBtn=()=>{sendBtn.disabled=!input.value.trim();}; input.addEventListener('input',syncSendBtn); input.addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();form.requestSubmit();}}); form.onsubmit=async(e)=>{e.preventDefault();const t=input.value.trim();if(!t)return;const ok=await ensureWsConnected(activeChatDeviceId);if(!ok||!state.ws||state.ws.readyState!==WebSocket.OPEN){alert('Нет соединения. Попробуйте обновить чат.');return;}const enc=await encryptText(t); state.ws.send(JSON.stringify({type:'message:new',roomId:state.roomId,...enc,notificationPreview:t.slice(0,80)})); input.value=''; syncSendBtn();}; syncSendBtn();}}
...
function buildNotificationText(sender,text,notif=state.notif){return notif.showText?(notif.hideSender?text:`${sender}: ${text}`):(notif.hideSender?'Новое сообщение':`${sender}: новое сообщение`);}
let notificationAudio=null;
function playNotificationSound({mine,incomingRoomId,inActiveVisibleChat}){if(!state.notif.enabled||!state.notif.sound||mine||inActiveVisibleChat)return; if(incomingRoomId===state.roomId&&document.visibilityState==='visible')return; if(!notificationAudio)notificationAudio=new Audio('data:audio/wav;base64,UklGRlQAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YTAAAAABAQEB'); notificationAudio.currentTime=0; notificationAudio.play().catch(()=>{});}
...
function notifyIncoming(sender,text,incomingRoomId,mine=false,inActiveVisibleChat=false){if(!state.notif.enabled||state.roomMute[incomingRoomId])return; if(document.visibilityState==='visible'&&incomingRoomId!==state.roomId){showInAppToast({roomId:incomingRoomId,roomName:state.roomNames[incomingRoomId],sender,text});} playNotificationSound({mine,incomingRoomId,inActiveVisibleChat});}
...
if('serviceWorker' in navigator){navigator.serviceWorker.addEventListener('message',async(event)=>{const data=event.data||{};if(data.type==='open-chat'&&data.roomId){const hasRoom=STORAGE.get(STORAGE.roomState(data.roomId));if(hasRoom){upsertChat(data.roomId,{});await openChat(data.roomId);}else{showChatsList();alert('Нет локального доступа к этому чату. Восстановите доступ по recovery-коду или invite-ссылке.');}}});}