...
function rememberMessageStatus(roomId,messageId,status,clientMessageId=null){const normalized=normalizeMessageStatus(status);try{if(window.FPMessageStore172){const record=window.FPMessageStore172.updateStatus(roomId,messageId,normalized,clientMessageId);return record?.status?strongerMessageStatus(normalized,record.status):normalized;}}catch{}const keys=[messageStatusKey(roomId,messageId),messageStatusKey(roomId,clientMessageId)].filter(Boolean);const uniqueKeys=[...new Set(keys)];let strongest=normalized;uniqueKeys.forEach((key)=>{const current=messageStatusByKey.get(key);if(!current||messageStatusRank(normalized)>messageStatusRank(current))messageStatusByKey.set(key,normalized);strongest=strongerMessageStatus(strongest,messageStatusByKey.get(key));});return strongest;}
function getEffectiveMessageStatus(roomId,message){let strongest=normalizeMessageStatus(message?.status);try{if(window.FPMessageStore172){const record=window.FPMessageStore172.get(roomId,message?.id)||window.FPMessageStore172.get(roomId,message?.client_message_id);if(record?.status)strongest=strongerMessageStatus(strongest,record.status);return strongest;}}catch{}const keys=[messageStatusKey(roomId,message?.id),messageStatusKey(roomId,message?.client_message_id)].filter(Boolean);keys.forEach((key)=>{if(messageStatusByKey.has(key))strongest=strongerMessageStatus(strongest,messageStatusByKey.get(key));});return strongest;}
