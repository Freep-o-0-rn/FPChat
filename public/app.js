...
function getMessageReplyMeta(messageId){const cached=messageCache.get(Number(messageId));if(cached)return{messageId:Number(messageId),author:cached.author||'Неизвестно',preview:cached.preview||'Сообщение недоступно',kind:'text'};return{messageId:Number(messageId),author:'Неизвестно',preview:'Сообщение недоступно',kind:'text'};}
...
const mediaFileInput=document.getElementById('mediaFileInput');const attachBtn=document.querySelector('.composer-attach');if(attachBtn&&mediaFileInput){attachBtn.onclick=(e)=>{e.preventDefault();mediaFileInput.click();};mediaFileInput.onchange=()=>{const files=Array.from(mediaFileInput.files||[]).slice(0,10);mediaFileInput.value='';if(!files.length)return;alert(`Выбрано файлов: ${files.length}. Полный media preview реализован на сервере/API и в следующем UI шаге.`);};}const form=document.getElementById('sendForm'),input=document.getElementById('msgInput'),sendBtn=document.getElementById('sendBtn'); if(form&&input&&sendBtn){const syncSendBtn=()=>{sendBtn.disabled=!input.value.trim();}; input.addEventListener('input',()=>{syncSendBtn();autoResizeMessageInput(input);const draft=ensureDraftState(state.roomId);draft.text=input.value;scheduleDraftSave(state.roomId);renderChats();}); input.addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();form.requestSubmit();}}); form.onsubmit=async(e)=>{e.preventDefault();const t=input.value.trim();if(!t)return;const ok=await ensureWsConnected(activeChatDeviceId);if(!ok||!state.ws||state.ws.readyState!==WebSocket.OPEN){alert('Нет соединения. Попробуйте обновить чат.');return;}const enc=await encryptText(t);const draft=ensureDraftState(state.roomId);const replyToMessageId=draft.replyTo?.messageId||null;state.ws.send(JSON.stringify({type:'message:new',roomId:state.roomId,...enc,notificationPreview:t.slice(0,80),replyToMessageId}));input.value='';draft.text='';draft.replyTo=null;updateReplyComposerBar();await clearDraftOnServer(state.roomId);syncSendBtn();autoResizeMessageInput(input);}; syncSendBtn();autoResizeMessageInput(input);await loadDraftForCurrentRoom();syncSendBtn();}}
function appendMessage(box,m,txt,mine,autoScroll=true){const w=document.createElement('div');w.className=`bubble-wrap msg ${mine?'mine':''}`;const isIncoming=!mine;const isRead=mine?1:(m.status==='read'?1:0);const replyMeta=m.reply_to_message_id?getMessageReplyMeta(m.reply_to_message_id):null;const replyHtml=replyMeta?`<button type="button" class="reply-block" data-reply-message-id="${replyMeta.messageId}"><div class="reply-block-author">${replyMeta.author}</div><div class="reply-block-preview">${replyMeta.preview}</div></button>`:'';w.innerHTML=`<div class='bubble'><div><b>${m.sender_name}</b></div>${replyHtml}<div>${txt}</div><div class='meta'>${formatMessageTime(m.created_at)} ${mine?deliveryIcon(m.status):''}</div></div>`;const bubble=w.querySelector('.bubble');const swipeIcon=document.createElement('div');swipeIcon.className='swipe-reply-icon';swipeIcon.textContent='↩';w.appendChild(swipeIcon);w.dataset.id=m.id;w.dataset.createdAt=m.created_at;w.dataset.messageId=String(m.id);w.dataset.incoming=isIncoming?'1':'0';w.dataset.read=String(isRead);w.querySelector('.reply-block')?.addEventListener('click',(e)=>{e.preventDefault();e.stopPropagation();const id=Number(e.currentTarget.dataset.replyMessageId);void findAndFocusReplyMessage(id);});w.addEventListener('contextmenu',(e)=>{e.preventDefault();e.stopPropagation();showMessageReplyMenu(m.id,e.clientX,e.clientY);});let swipeState=null;w.addEventListener('touchstart',(e)=>{if(!isMobileViewport())return;const touch=e.touches?.[0];if(!touch)return;swipeState={startX:touch.clientX,startY:touch.clientY,dx:0,canceled:false,tracking:true};w.classList.remove('swipe-reset');},{passive:true});w.addEventListener('touchmove',(e)=>{if(!swipeState?.tracking||swipeState.canceled)return;const touch=e.touches?.[0];if(!touch)return;const dx=touch.clientX-swipeState.startX;const dy=touch.clientY-swipeState.startY;swipeState.dx=dx;if(Math.abs(dy)>SWIPE_CANCEL_VERTICAL){swipeState.canceled=true;bubble.style.transform='translateX(0px)';swipeIcon.style.opacity='0';swipeIcon.style.transform='translateY(-50%) scale(.8)';return;}if(dx<0){const swipeDistance=Math.abs(dx);const progress=Math.min(swipeDistance/SWIPE_REPLY_THRESHOLD,1);bubble.style.transform=`translateX(-${Math.min(swipeDistance,72)}px)`;swipeIcon.style.opacity=String(progress);swipeIcon.style.transform=`translateY(-50%) scale(${0.8+progress*0.2})`;w.classList.add('swiping');}}, {passive:true});w.addEventListener('touchend',()=>{if(!swipeState){return;}const shouldReply=!swipeState.canceled&&swipeState.dx<=-SWIPE_REPLY_THRESHOLD;w.classList.remove('swiping');w.classList.add('swipe-reset');bubble.style.transform='translateX(0px)';swipeIcon.style.opacity='0';swipeIcon.style.transform='translateY(-50%) scale(.8)';if(shouldReply){setSelectedReply(state.roomId,getMessageReplyMeta(m.id));navigator.vibrate?.(10);}swipeState=null;});w.addEventListener('touchcancel',()=>{swipeState=null;w.classList.remove('swiping');w.classList.add('swipe-reset');bubble.style.transform='translateX(0px)';swipeIcon.style.opacity='0';swipeIcon.style.transform='translateY(-50%) scale(.8)';});box.appendChild(w);const previewText=txt==='[cannot decrypt]'?'Сообщение недоступно':makeReplyPreview(txt);messageCache.set(Number(m.id),{id:Number(m.id),author:m.sender_name,text:txt,preview:previewText,kind:'text'});if(isIncoming&&!isRead){if(!autoScroll){pendingIncomingReadIds.push(Number(m.id));}observeUnreadMessage(w);}if(autoScroll)box.scrollTop=box.scrollHeight;}
function sendClientState(){if(!state.ws||state.ws.readyState!==WebSocket.OPEN)return;state.ws.send(JSON.stringify({type:'client:state',activeRoomId:state.roomId||null,visible:document.visibilityState==='visible'}));}
function connectWs(deviceId){setLocalConnectionState('connecting');if(state.ws)state.ws.close();const p=location.protocol==='https:'?'wss':'ws';const currentSeq=++appWsSeq;activeAppWsSeq=currentSeq;const ws=new WebSocket(`${p}://${location.host}?device=${encodeURIComponent(deviceId)}`);ws.deviceId=deviceId;state.ws=ws;const isCurrentWs=()=>state.ws===ws&&activeAppWsSeq===currentSeq;wsConnectStartedAt=Date.now();ws.onopen=()=>{if(!isCurrentWs())return;setLocalConnectionState('connected');sendClientState();};ws.onerror=()=>{if(!isCurrentWs())return;setLocalConnectionState('disconnected');};ws.onclose=()=>{if(!isCurrentWs())return;setLocalConnectionState('disconnected');};ws.onmessage=async(ev)=>{if(!isCurrentWs())return;const payload=JSON.parse(ev.data);if(payload.type==='message:new'){const roomId=payload.roomId; if(!roomId)return; const chat=state.chats.find(c=>c.roomId===roomId)||{};const txt=await decryptText(payload.message.iv,payload.message.ciphertext).catch(()=>"[cannot decrypt]");const mine=payload.message.sender_device_id===deviceId;const box=document.getElementById('messages');const inActiveChat=state.roomId===roomId&&box;const nearBottom=inActiveChat?isMessagesAtBottom():false;const messageId=payload.message.id;const messageSelector=`[data-message-id="${messageId}"], [data-id="${messageId}"]`;const hasMessageInDom=Boolean(document.querySelector(messageSelector));const normalizedLast={lastMessage:txt,lastSender:payload.message.sender_name,lastActivity:payload.message.created_at||new Date().toISOString()};if(mine){upsertChat(roomId,{...normalizedLast,unread:chat.unread||0});if(inActiveChat&&!hasMessageInDom){appendDateSeparatorIfNeeded(box,payload.message.created_at);appendMessage(box,payload.message,txt,true,true);box.scrollTop=box.scrollHeight;}}else if(inActiveChat){if(!hasMessageInDom){appendDateSeparatorIfNeeded(box,payload.message.created_at);appendMessage(box,payload.message,txt,false,nearBottom);}if(nearBottom){markIncomingMessagesRead(roomId,deviceId,[messageId]);upsertChat(roomId,{...normalizedLast,unread:0});}else{const incomingId=Number(messageId);if(Number.isInteger(incomingId)&&incomingId>0){pendingIncomingReadIds=[...new Set([...pendingIncomingReadIds,incomingId])];}const unreadCount=pendingIncomingReadIds.length;upsertChat(roomId,{...normalizedLast,unread:unreadCount});renderNewMessagesPill(unreadCount);}}else{const unread=(chat.unread||0)+1;upsertChat(roomId,{...normalizedLast,unread});updateUnreadPresentation();}
