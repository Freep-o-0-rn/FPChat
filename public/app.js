...
function syncPendingIncomingReadIds(){pendingIncomingReadIds=[...new Set(pendingIncomingReadIds.map(Number).filter(Boolean))];}
...
function updateUnreadIndicators(){const box=document.getElementById('messages');const pill=document.getElementById('newMessagesPill');if(!box||!pill||!state.roomId)return;syncPendingIncomingReadIds();const count=pendingIncomingReadIds.length;const atBottom=isMessagesAtBottom();pill.classList.toggle('hidden',count===0||atBottom);if(count>0&&!atBottom)pill.textContent=formatUnreadLabel(count);upsertChat(state.roomId,{unread:atBottom?0:count});renderChats();updatePushBadge();}
...
function markMessageRead(messageId){const id=Number(messageId);if(!id)return;const box=document.getElementById('messages');const msgEl=box?.querySelector(`.msg[data-message-id="${id}"]`);if(msgEl&&msgEl.dataset.read==='0')msgEl.dataset.read='1';const before=pendingIncomingReadIds.length;pendingIncomingReadIds=pendingIncomingReadIds.filter((x)=>Number(x)!==id);if(before!==pendingIncomingReadIds.length){markIncomingMessagesRead(state.roomId,activeChatDeviceId,[id]);}updateUnreadIndicators();}
...
const box=document.getElementById('messages'); unreadVisibleObserver=new IntersectionObserver((entries)=>{entries.forEach((entry)=>{if(!entry.isIntersecting)return;const el=entry.target;markMessageRead(el.dataset.messageId);unreadVisibleObserver?.unobserve(el);});},{root:box,threshold:0.01}); let prev=''; messages.forEach(async m=>{const d=new Date(m.created_at).toDateString(); if(d!==prev){prev=d; const sep=document.createElement('div');sep.className='date-sep';sep.textContent=d===new Date().toDateString()?'Сегодня':d;box.appendChild(sep);} const mine=m.sender_device_id===deviceId; const txt=await decryptText(m.iv,m.ciphertext).catch(()=>"[cannot decrypt]"); appendMessage(box,m,txt,mine,false);}); box.scrollTop=box.scrollHeight;updateUnreadIndicators();
box.addEventListener('scroll',()=>updateUnreadIndicators());
