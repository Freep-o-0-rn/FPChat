...
function appendMessage(box,m,txt,mine,autoScroll=true){
  ...
  box.appendChild(w);
  window.FPReactionRenderer188?.mount?.(w,state.roomId,m);
...
function handleWsPresenceUpdate(payload){if(payload?.type!=='presence:update'||!payload.deviceId)return false;const roomId=String(payload.roomId||'');if(!roomId||roomId!==String(state.roomId||''))return true;const previousPresence=state.presence[payload.deviceId]||{};state.presence[payload.deviceId]={participantId:Number(payload.participantId)||Number(previousPresence.participantId)||null,roomId,deviceId:payload.deviceId,displayName:payload.displayName,online:Boolean(payload.online),lastSeenAt:payload.lastSeenAt||null,presenceState:payload.presenceState||null,statusUnavailable:payload.statusUnavailable===true};renderPresenceStatus();return true;}
