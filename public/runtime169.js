...
(() => {
  ...
  // Build 186.1: bounded, explicit observations. No transport/timer/crypto
  ...
  const loading186 = (() => {
    ...
    function fail(token, stage, error, httpStatus) {
      ...
      item.error = {
        stage: ['queue', 'fetch', 'response', 'body', 'buffer', 'decrypt', 'element', 'history', 'key', 'join', 'render'].includes(stage) ? stage : 'unknown',
    ...
    function report() {
      ...
      return {
        schema:1, build:'186.1', mode:'passive-owner-hooks', installedAtMs:round(startedAt), enabled, limit:LIMIT, dropped, activeElementWatches:watches.size,
        coverage:{boot:'explicit-gate-marks-since-inline-loader; end marks can be timeout, see completed', room:'hooks from runtime installation; ordinary-entry; direct-entry-starts-after-join', media:'common readEncryptedMedia174 path including voice/save reads; no independent I/O or voice/save playback readiness', cache:'CacheStorage owner only; HTTP cache not inferred', display:'image load, active video loadeddata, neighbor video loadedmetadata and frame opportunity; not native decode duration or actual paint', resources:'partial browser buffer; bounded runtime history; zero bytes do not prove a cache hit', privacy:'local trace numbers only; no text, room/media/device/message IDs, URLs, keys, error messages or request bodies'},
        ...
        records:[...records.values()].map(item => ({...item, startMs:round(item.startMs), points:{...item.points}, counts:{...item.counts}, ...(item.error ? {error:{...item.error}} : {}), stagesMs:{queue:duration(item,'queue-start','slot-ready'), cache:duration(item,'cache-start','cache-ready'), responseAfterAdmission:duration(item,'slot-ready','response-ready'), body:duration(item,'body-start','body-ready'), buffer:duration(item,'buffer-start','buffer-ready'), decrypt:duration(item,'decrypt-start','decrypt-ready'), element:duration(item,'url-ready','element-ready'), key:duration(item,'key-start','key-ready'), join:duration(item,'join-start','join-ready'), history:duration(item,'history-start','history-ready'), render:duration(item,'render-start','text-ready'), draft:duration(item,'draft-start','draft-ready'), scroll:duration(item,'scroll-start','scroll-ready')}}))
      };
    }
    function reset() { for (const id of watches.keys()) cleanup(id); records.clear(); dropped = 0; }
    function download() {
      const url = URL.createObjectURL(new Blob([JSON.stringify(report(), null, 2)], {type:'application/json'}));
      const link = document.createElement('a'); link.href = url; link.download = 'FPChat-186.1-loading.json';
