...
echo.
echo ========================================
echo        FPChat safe update, build 171
echo ========================================
echo.
...
powershell -NoProfile -Command "$c=@(Get-NetTCPConnection -LocalPort %APP_PORT% -State Listen -ErrorAction SilentlyContinue); if($c.Count > 0){exit 1}; exit 0" >nul 2>&1
if errorlevel 1 (
    set "SERVER_WAS_RUNNING=0"
