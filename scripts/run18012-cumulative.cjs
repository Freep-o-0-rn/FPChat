...
for(const file of files){
  ...
  let status='PASS';
  if(child.error?.code==='ETIMEDOUT'||child.signal==='SIGTERM')status='TIMEOUT';
  else if(child.error||child.status!==0)status='FAIL';
  const result={
    file,
    status,
...
if(!probe&&(counts.FAIL||counts.TIMEOUT))process.exitCode=1;
