...
assert(app.includes("permission === 'prompt' && previouslyGranted"), 'repeat-prompt detection missing');
...
assert(voice.includes('Разрешить при посещении сайта'), 'persistent permission guidance missing');

assert.equal(version.build, '182.3');
assert(updater.includes('set "EXPECTED_BUILD=182.3"'), 'safe updater build gate does not match Build 182.3');
...
console.log('PASS updater/version gate matches Build 182.3');
